    ...
    <div class="card shadow border-0">
        ...
        <div class="card-header text-white d-flex justify-content-between align-items-center">
            <h4 class="mb-0">
                ...
                {{ isset($quotation) ? 'ฟอร์มแก้ไขใบเสนอราคา' : 'ฟอร์มสร้างใบเสนอราคา' }}
            </h4>
        </div>

        <div class="card-body">

<form action="{{ isset($quotation) ? '/quotation/'.$quotation->quotation_id : '/quotation' }}" method="post">
@csrf
@if(isset($quotation))
    @method('PUT')
@endif
...
<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="text" class="form-control" name="quotation_no" placeholder="เว้นว่างเพื่อออกเลขอัตโนมัติ"
            value="{{ old('quotation_no', $quotation->quotation_no ?? $nextNo ?? '') }}"
            {{ isset($quotation) ? 'readonly' : '' }}>
    ...
    <div class="col-md-4">
        ...
        <input type="date" class="form-control" name="quotation_date"
            value="{{ old('quotation_date', isset($quotation->quotation_date) ? \Carbon\Carbon::parse($quotation->quotation_date)->format('Y-m-d') : '') }}">
    ...
    <div class="col-md-4">
        ...
        <input type="number" class="form-control" name="price_validity_days" min="0"
            value="{{ old('price_validity_days', $quotation->price_validity_days ?? 30) }}">
        ...
        <div class="col">
            <select class="form-select" name="customer_id" id="customer_id">
                ...
                @foreach($customers as $cus)
                    <option value="{{ $cus->customer_id }}" {{ (old('customer_id', $quotation->customer_id ?? session('new_customer_id')) == $cus->customer_id) ? 'selected' : '' }}>
                        {{ $cus->customer_name }}
                    </option>
                @endforeach
            </select>
...
<div class="mb-3">
    ...
    <input type="text" class="form-control" name="subject"
        value="{{ old('subject', $quotation->subject ?? 'ขอเสนอราคาสินค้าดังรายการต่อไปนี้') }}">
...
<div class="mb-3">
    ...
    <div id="item-rows">
        @php
            $existingItems = old('items', isset($quotation) ? $quotation->details->map(function ($d) {
                return [
...
<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" min="0" max="100" class="form-control" name="vat_rate"
            value="{{ old('vat_rate', $quotation->vat_rate ?? 7) }}">
    ...
    <div class="col-md-8">
        ...
        <input type="text" class="form-control" name="payment_terms"
            value="{{ old('payment_terms', $quotation->payment_terms ?? 'ตามหลักเกณฑ์ของทางราชการ') }}">
...
<div class="d-flex gap-2 mt-4 justify-content-end">
    @if(isset($quotation))
        <a href="/quotation/{{ $quotation->quotation_id }}/print" target="_blank" class="btn btn-outline-primary">
            <i class="fas fa-print me-1"></i> พิมพ์ใบเสนอราคา
        </a>
    @endif
    <button type="submit" class="btn btn-submit-gradient">{{ isset($quotation) ? 'บันทึกการแก้ไข' : 'บันทึกใบเสนอราคา' }}</button>
